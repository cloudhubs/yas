...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http, SecurityProperties securityProps) throws Exception {
        ...
        // 1. Apply Dynamic CSRF Exclusions
        if (securityProps.getCsrfIgnored() != null && !securityProps.getCsrfIgnored().isEmpty()) {
            http.csrf(csrf -> csrf.ignoringRequestMatchers(
                securityProps.getCsrfIgnored().toArray(new String[0])
            ));
        }
        ...
        return http
            ...
            // 3. Re-attach the Keycloak JWT Converter
